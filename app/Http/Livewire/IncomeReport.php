...
class IncomeReport extends Component
{
    ...
    public function mount()
    {
        $this->months =
            ...
            ->where('source_type', 'App\Models\Category')
            ->groupBy('month')
            ->orderBy('month')
            ->get();
    }
    ...
    public function render()
    {
        $categories = Category::query()
            ...
            ->where('type', 'income')
            ->withSum(['outgoingTransactions' => function ($q) {
        ...
        foreach ($parent_categories as $index => $parent_category) {
            ...
            $final_list[$index]['amount'] = $parent_category->outgoing_transactions_sum_amount;
            foreach ($sub_categories as $sub_index => $sub_category) {
                if ($sub_category->category_id == $parent_category->id) {
                    $final_list[$index]['amount'] += $sub_category->outgoing_transactions_sum_amount;
                    $final_list[$index]['sub_categories'][$sub_index] = [
                        'id' => $sub_category->id,
                        'name' => $sub_category->name,
                        'amount' => $sub_category->outgoing_transactions_sum_amount,
                    ];
                }
            }
        }
