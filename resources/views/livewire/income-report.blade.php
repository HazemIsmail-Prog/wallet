<div
    ...
    <div class="p-4 flex items-center justify-between dark:border-green-400 border-green-600 border-b">
        <div class=" text-green-600 dark:text-green-400 uppercase font-extrabold  ">
            <div >income</div>
            <div>{{ number_format($categories->sum('amount')/session('current_country')->factor,session('current_country')->decimal_points) }} <span class=" font-extralight text-xs">{{ session('current_country')->currency }}</span></div>
        </div>
